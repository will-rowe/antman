...
#define multiShift 27

// seed for gerenerating multiple hash values
#define multiSeed 0x90b45d39fb6da1fa
...
static inline uint64_t NTF64(const uint64_t fhVal, const unsigned k, const unsigned char charOut, const unsigned char charIn)
{
...
static inline uint64_t NTR64(const uint64_t rhVal, const unsigned k, const unsigned char charOut, const unsigned char charIn)
{
...
static inline uint64_t NTC64(const unsigned char charOut, const unsigned char charIn, const unsigned k, uint64_t *fhVal, uint64_t *rhVal)
{
...
static inline bool NTMC64(const char *kmerSeq, const unsigned int k, const unsigned int m, uint64_t *fhVal, uint64_t *rhVal, unsigned int *locN, uint64_t *hVal)
{
    *fhVal = *rhVal = 0;
...
static inline void NTMC64s(const unsigned char charOut, const unsigned char charIn, const unsigned k, const unsigned m, uint64_t *fhVal, uint64_t *rhVal, uint64_t *hVal)
{
    uint64_t bVal = 0, tVal = 0;
...
/*****************************************************************************
 * ntInit will initiate a ntHash iterator.
 * 
 * arguments:
 *      seq                       - pointer to the sequence that needs hashing
 *      seqLen                    - the length of the sequence
 *      kSize                     - the k-mer size to use
 *      numHash                   - the number of hashes to return per k-mer
 * 
 * returns:
 *      an initiated ntHash iterator
 * 
 * note:
 *      the user must check and free the returned iterator
 *      the user must free the supplied sequence
 */
nthash_iterator_t *ntInit(char *seq, unsigned int seqLen, unsigned int kSize, unsigned int numHash)
{
    ...
    nt->seq = seq;
    nt->kSize = kSize;
    nt->numHashes = numHash;
    nt->numKmers = (seqLen - kSize + 1);

    // set up the init values
    nt->seqIterator = 0;
    nt->m_fhVal = 0;
    nt->m_rhVal = 0;

    // init hashVector for hashes
    nt->hashVector = calloc(sizeof(uint64_t), (nt->numKmers * nt->numHashes));
    if (!nt->hashVector)
    {
        free(nt);
        return NULL;
...
/*****************************************************************************
 * ntPrepare will set the internal state of an initialised ntHash iterator.
 * 
 * arguments:
 *      nt      - the ntHash iterator
 */
void ntPrepare(nthash_iterator_t *nt)
{
    assert(nt);
    unsigned int locN = 0;
    while ((nt->seqIterator < nt->numKmers) && (!NTMC64(nt->seq + nt->seqIterator, nt->kSize, nt->numHashes, &nt->m_fhVal, &nt->m_rhVal, &locN, nt->hashVector)))
        nt->seqIterator += locN + 1;
    if (nt->seqIterator >= nt->numKmers)
        nt->seqIterator = SIZE_MAX;
}

/*****************************************************************************
 * ntIterate will relpace the current hash value(s) with that of the next
...
void ntIterate(nthash_iterator_t *nt)
{
    ...
    if (seedTab[(unsigned char)(nt->seq[nt->seqIterator + nt->kSize - 1])] == seedN)
    {
        nt->seqIterator += nt->kSize;
        ntPrepare(nt);
    }
    else
        NTMC64s(nt->seq[nt->seqIterator - 1], nt->seq[nt->seqIterator - 1 + nt->kSize], nt->kSize, nt->numHashes, &nt->m_fhVal, &nt->m_rhVal, nt->hashVector);
    return;
}

/*****************************************************************************
 * ntDestroy will clear an ntHash iterator and release the memory.
 * 
 * arguments:
 *      nt      - the ntHash iterator
 * 
